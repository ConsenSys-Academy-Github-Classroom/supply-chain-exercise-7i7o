...
contract SupplyChain {
  ...
  modifier checkValue(uint _sku) {
    ...
    if (amountToRefund > 0) {
      ...
      require (sent, 'Failed to transfer change to buyer');
    }
  }
  ...
  constructor() public {
    ...
    // skuCount = 0; // Not necessary because uint is initialized in 0
  }

  function addItem(string memory _name, uint _price) public returns (bool) {
    require (_price > 0 , 'Item price should be greater than 0');
    // require (_name.length > 0, 'Item should have a name');
  ...
  function buyItem(uint sku) public payable forSale(sku) paidEnough(sku) checkValue(sku) { // 1. & 5.
    (bool sent, bytes memory data) = items[sku].seller.call.value(items[sku].price)(""); // 2.
    require (sent, 'Failed to pay the seller');
    items[sku].buyer = msg.sender; // 3.
